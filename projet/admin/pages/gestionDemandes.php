        ...
        <table class="well col-lg-12 table-bordered table-condensed table-hover">
            ...
            for ($i = 0; $i < $length; $i++) {
                ...
                <tr>
                    ...
                    <td class="text-center"><button class="btn btn-info" value="" OnClick='window.location.href="../pages/accueilAdmin.php?page=acceptConge&id=<?php echo $data[$i]->__get('id_conges')?>&val=1"'><span class="glyphicon glyphicon-ok"></span></button></td>
                    <td class="text-center"><button class="btn btn-danger" value="" OnClick='window.location.href="../pages/accueilAdmin.php?page=acceptConge&id=<?php echo $data[$i]->__get('id_conges')?>&val=-1"'><span class="glyphicon glyphicon-remove"></span></button></td>
                <?php }
                ?>
        </table>
    </div>
...
<script>
function accept(id){
    var result = confirm("Voulez-vous vraiment accepter?");
...
function notAccept(id){
    var result = confirm("Voulez-vous vraiment refuser?");
    if(result){
       console.log("OK DEL"); 
    }else{
        console.log("NOK DEL");
